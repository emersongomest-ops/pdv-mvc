...
| **RN-073** | `POST` complete-sale and create-refund **require** `Idempotency-Key` (max 128). Same key + same payload → safe replay of stored response (`Idempotent-Replayed: true`). Same key + different payload → `IDEMPOTENCY_KEY_REUSE` (409). Concurrent in-flight claim → `IDEMPOTENCY_REQUEST_IN_FLIGHT` (409). Failed attempts release the claim so the same key can retry. Scope is `{route}:{saleId}` + acting `user_id`. Retention **7 days** (configurable `IDEMPOTENCY_RETENTION_DAYS`); daily `idempotency:purge` deletes older rows. | Required |
