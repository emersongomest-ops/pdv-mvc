...
- **Consequences:**
  ...
  - On MFA confirm, eight one-time recovery codes are shown once (bcrypt hashes in `mfa_recovery_codes`, encrypted at rest). Admin MFA reset: `POST /api/admin/users/{id}/mfa/reset` (RN-074).
