...
- **Financial mutation idempotency (RN-073):** `IdempotencyGuard` + `idempotency_records` on `POST .../sales/{id}/complete` and `POST /api/admin/sales/{id}/refunds`. Required `Idempotency-Key`; claim via unique `(scope, key)`; replay same hash; conflict on hash mismatch / in-flight. Correlates with `X-Request-Id` on the row. Payment webhooks remain idempotent via `payment_webhook_events` (RN-054).
