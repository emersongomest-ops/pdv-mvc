...
| Secrets (`.env` + `.env.docker`) | **Separate** | Compose interpolation (`.env`) + Laravel `env_file`; `APP_KEY`, `CUSTOMER_PII_*` — without them ciphertext is unusable (ADR-0008). Rotation: [`key-rotation.md`](./key-rotation.md). |
